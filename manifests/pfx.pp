...
define windows_certs::pfx ($cert_name = $title,$cert_stage_location,$cert_secure_pass,$cert_store = "WebHosting",$source_location){
  ...
  validate_absolute_path($cert_location)
  ...
  exec{"Insert-PFX-Cert-${cert_name}":
      command   => "Import-Module WebAdministration; Import-PfxCertificate -FilePath ${$cert_location} -CertStoreLocation Cert:\\LocalMachine\\${cert_store} -Password (\"${$cert_secure_pass}\" | ConvertTo-SecureString -Key @(1..32))",
